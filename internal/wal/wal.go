...
func (l *Log) load() error {
	// 读取wal日志的工作目录，注意读取出来的内容是按照文件名的顺序进行排序的
	...
	if startIdx != -1 {
		...
		// 清空所有start文件之前的文件，已经写入的数据保留日志文件是没有意义的
	...
	if endIdx != -1 {
		...
		if len(l.segments) > 1 && l.segments[len(l.segments)-2].index == l.segments[len(l.segments)-1].index {
	...
	// 将文件指针移动到文件的末尾，因为后续的写入肯定都是在最后一个文件进行的
...
func (l *Log) writeBatch(b *Batch) error {
	...
	datas := b.datas
	for i := 0; i < len(b.entries); i++ {
		// 批量写入时，数据都是在同一个变量之中的，需要用size将数据取出来
...
// loadSegmentEntries 每一个日志文件又是由每一个日志条目组成的，此处用于加载一个文件的全部日志条目
